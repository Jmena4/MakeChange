...
public class MakeChange {
	...
	//call the getChange() method and calculate the correct denominations to print out
	public static void getChange(double changeOwed) {
		double changeDenominations[] = {20.00 ,10.00 ,5.00 , 1.00, .25, .10, .05, .01};
		String changeBills[]= {"twenty dollar bill", "ten dollar bill", "five dollar bill", "one dollar bill", "quater", "dime", "nickel", "penn"};
		int counter;
		...
		for ( int i = 0; i < changeDenominations.length; i++) {
			...
			while (balance >= changeDenominations[i]) {
				balance =  Math.round(100.0*(balance) )/100.0;
				balance -= changeDenominations[i];
				counter++;
//				System.out.println(balance);
				//  attempted to change the ending of
				if (counter > 1 && changeBills[i] == "penn") {
					changeBills[i] = changeBills[i] + "ies";

				}
				else if (counter == 1 && changeBills[i] == "penn") {
					changeBills[i] = changeBills[i] + "y";
				}
//				if (counter >= 2 && changeBills[i] != "penn" ) {
//					changeBills[i] = changeBills[i] + "s";
//				}
